...
from metaevobox.environment.basic_environment import PBO_Env
from metaevobox.environment.parallelenv import *
from metaevobox.logger import *
import copy
from metaevobox.environment.problem.utils import *
import numpy as np
import os, warnings
import matplotlib
import matplotlib.pyplot as plt
from metaevobox.rl.utils import save_class
...
class Trainer(object):
    ...
    def __init__(self, config, user_agent, user_optimizer, user_datasets):
        ...
        if self.config.train_parallel_mode == 'subproc' and self.agent.__str__() in ['B2OPT', 'GLHF', 'Surr_RLDE',
                                                                                     'RNNOPT'] and self.config.device != 'cpu':
            warnings.warn(
                "Subproc training parallel mode for MetaBBO optimizers using CUDA will lead to CUDA kernel errors, changed to dummy.",
        ...
        while not is_end:
            ...
            with tqdm(range(int(np.ceil(self.train_set.N / self.train_set.batch_size))),
                ...
                for problem_id, problem in enumerate(self.train_set):
                    ...
                    if self.config.train_mode == "single":
                        env_list = [PBO_Env(copy.deepcopy(problem[0]), copy.deepcopy(self.optimizer)) for _ in
                                    range(bs)]  # bs
                    elif self.config.train_mode == "multi":
